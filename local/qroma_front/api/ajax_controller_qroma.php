...
try {
    ...
    switch ($_REQUEST['request_type']) {
        ...
        case 'obtenerUsuariosPanel3':
            ...
            break;
        case 'seguimientoFirmaPanel1':
            $returnArr = seguimientoFirmaPanel1();
            break;
        case 'seguimientoFirmaPanel2':
            $returnArr = seguimientoFirmaPanel2($_POST['courseId']);
            break;
        case 'seguimientoFirmaPanel3':
            $returnArr = seguimientoFirmaPanel3($_POST);
            break;
    }
} catch (Exception $e) {
...
function seguimientoFirmaPanel1() {
    global $DB;

    $courses = $DB->get_records_sql("select course from {firma_detalle} group by course");

    foreach($courses as $c) {
        $record = $DB->get_record_sql("select course, COUNT(userid) as cant_usuarios from {firma_detalle}
                where timecreated in(SELECT MAX(timecreated) as createdtime FROM {firma_detalle}
                WHERE course = ? GROUP BY userid)
                group by course", array($c->course));

        $course = $DB->get_record_sql("select fullname FROM {course}
                WHERE id = ? ", array($c->course));

        $cantUsuarios = $record->cant_usuarios ?? 0;

        $return[] = [
            'name'=> $course->fullname ?? '',
            'id'=> $c->course ?? '',
            'compromiso'=> $cantUsuarios . ' usuarios han firmado'
        ];
    }
...
function seguimientoFirmaPanel2($courseId) {
    global $DB;

    $courses = $DB->get_records_sql("select * from {firma_detalle}
                where timecreated in(SELECT max(timecreated) as createdtime FROM {firma_detalle}
                WHERE course = ? GROUP BY userid) ORDER BY timecreated DESC", array($courseId));

    foreach ($courses as $c) {
        $fecha = date('Y-m-d', $c->timecreated);
        $list[$fecha][] = $c->userid;
    }

    foreach($list as $key=>$l) {
        $compromiso = count($l) . ' usuarios han firmado';
        if(count($l) == 1) {
            $compromiso = count($l) . ' usuario ha firmado';
        }
...
function seguimientoFirmaPanel3($values) {
    global $DB;

    $courseId = $values['courseId'];
    $fecha = $values['fecha'];

    $detail = $DB->get_records_sql("select * from {firma_detalle}
                where timecreated in(SELECT max(timecreated) as createdtime FROM {firma_detalle}
                WHERE course = ? GROUP BY userid) ORDER BY timecreated DESC", array($courseId));

    foreach($detail as $d) {
        if($fecha != date('Y-m-d', $d->timecreated)) {
            continue;
        }

        $userArr = $DB->get_record_sql("select username from {qroma_user_tmp} where userid = ?", array($d->userid));
